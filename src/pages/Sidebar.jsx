...
const Sidebar = ({
  ...
  const visibleRoutes = routes.filter((r) => {
    ...
    // If route requires superadmin but user is not superadmin
    if (r.role === "superadmin") return false;

    // If route requires a specific permission, check if user has it
    if (r.permission) {
      return user?.permissions?.includes(r.permission);
    }

    return true; // No special role/permission required (e.g., Dashboard, Profile)
  });
